...
public class IPv6Packet {
	...
	byte [] source = new byte[16]; // 16byte long source IPv6 address
	byte [] dest = new byte[16]; // 16byte long source IPv6 address
	...
	final int PACKET_SPECIFIC_DATA_LOC = 4; /* this is where packet s
	...
	public void parseIPv6Packet(RawPacket packet) {
		...
		try {
			...
			wrapper.get(udpPayload, 0, udpPayload.length);
